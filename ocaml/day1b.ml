...
let parse_int input_ch =
  ...
  match line with None -> failwith "Not found" | Some x -> Int.of_string x

let contains set el = Set.exists set ~f:(fun x -> x = el)

let contains_complement set num el =
  el <> 2020 - num && contains set (2020 - num - el)

let check_sum set num = Set.find set ~f:(contains_complement set num)

let rec find_sum_triplet ?(seen = Set.empty (module Int)) input_ch =
...
let solve filename = In_channel.with_file filename ~f:find_sum_triplet

let () = "../data/day1b.txt" |> solve |> printf "%d\n"
